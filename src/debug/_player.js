...
Perso.src = 'src/assets/sacha.png';

const switchChar = function () {
  const inputs = window.document.getElementsByName('Personnage');
  for (i = 0; inputs[i]; i++) {
    if (inputs[i].checked) {
  ...
  if (character == 'sacha') {
    ...
    Perso.src = 'src/assets/pikachu.png';
  }
  switch (key) {
    case 37:
...
const deplacementy = function (arg) {
  animation = 1;
...
const deplacementx = function (arg) {
  animation = 1;
  console.log(animation);
  setTimeout(() => {
    animation = 0;
...
const mur = function (direction, x, y, map) {
  let i, j, val;
  switch (direction) {
    case 37:
      x -= 16;
      ...
      i = Math.ceil(y / 16);
      if (
        (y % 16 == 2 && map[i - 1][j] >= 9 && map[i + 1][j] <= 21) ||
        (map[i][j] >= 9 && map[i][j] <= 21)
      ) {
        val = true;
    ...
    case 38:
      if (y !== 0) {
        ...
        i = Math.ceil(y / 16);
        if (
          (x % 16 == 8 && map[i][j - 1] >= 9 && map[i][j - 1] <= 21) ||
          (map[i][j] >= 9 && map[i][j] <= 21)
        ) {
          val = true;
